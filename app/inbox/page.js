  ...
  return (
    <AppShell pageTitle="HR Inbox">
      ...
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:16}}>
        <InboxPanel title="Missing documents" items={inbox.missingDocs} tone="danger" linkHref="/documents" renderItem={d => <><div style={{flex:1}}><div style={{fontSize:12,fontWeight:500}}>{d.worker_name}</div><div style={{fontSize:10,color:'var(--hint)'}}>{d.worker_number} · {d.document_type}</div></div><StatusBadge label="missing" tone="danger" /></>} />
        <InboxPanel title="Expired documents" items={inbox.expiredDocs} tone="danger" linkHref="/documents" renderItem={d => <><div style={{flex:1}}><div style={{fontSize:12,fontWeight:500}}>{d.worker_name}</div><div style={{fontSize:10,color:'var(--hint)'}}>{d.worker_number} · {d.document_type}</div></div><StatusBadge label="expired" tone="danger" /></>} />
        <InboxPanel title="Expiring documents" items={inbox.expiringDocs} tone="warning" linkHref="/documents" renderItem={d => <><div style={{flex:1}}><div style={{fontSize:12,fontWeight:500}}>{d.worker_name}</div><div style={{fontSize:10,color:'var(--hint)'}}>{d.worker_number} · {d.document_type}</div></div><span className="value" style={{fontSize:11}}>{formatDate(d.expiry_date)}</span></>} />
        <InboxPanel title="Contracts due" items={inbox.contractsDue} tone="warning" linkHref="/documents" renderItem={d => <><div style={{flex:1}}><div style={{fontSize:12,fontWeight:500}}>{d.worker_name}</div><div style={{fontSize:10,color:'var(--hint)'}}>{d.worker_number} · contract</div></div><span className="value" style={{fontSize:11}}>{formatDate(d.expiry_date)}</span></>} />
        <InboxPanel title="Expired certifications" items={inbox.expiredCerts} tone="danger" linkHref="/certifications" renderItem={c => <><div style={{flex:1}}><div style={{fontSize:12,fontWeight:500}}>{c.worker_name}</div><div style={{fontSize:10,color:'var(--hint)'}}>{c.worker_number} · {c.certification_type}</div></div><StatusBadge label="expired" tone="danger" /></>} />
        <InboxPanel title="Expiring certifications" items={inbox.expiringCerts} tone="warning" linkHref="/certifications" renderItem={c => <><div style={{flex:1}}><div style={{fontSize:12,fontWeight:500}}>{c.worker_name}</div><div style={{fontSize:10,color:'var(--hint)'}}>{c.worker_number} · {c.certification_type}</div></div><span className="value" style={{fontSize:11}}>{formatDate(c.expiry_date)}</span></>} />
