...
import { getPackCoverage, getDocuments, getWorkers, getCertifications, getWorker } from '../../lib/mockStore'
...
export default function PacksPage() {
  ...
  const openPackBuilder = (workerId) => {
    ...
    const docs = getDocuments().filter(d => d.worker_id === workerId && PACK_DOCUMENT_TYPES.includes(d.document_type))
  ...
  const handleBuildZip = async () => {
    ...
    try {
      ...
      // Cover page always included first
      ...
      const manifest = `WORKER PACK MANIFEST\n${'='.repeat(40)}\nWorker: ${selectedWorker.full_name}\nID: ${selectedWorker.worker_number}\nPack generated: ${new Date().toLocaleString()}\nGenerated by: Innovation Technologies LLC O.P.C.\n\nDOCUMENTS INCLUDED (${selected.length + 1}):\n00. COVER PAGE (auto-generated HTML — open in browser to print/save as PDF)\n${selected.map((s,i)=>`${String(i+1).padStart(2,'0')}. ${s.name}`).join('\n')}\n`
          ...
          <div className="panel">
            ...
            <div style={{display:'flex',alignItems:'center',gap:8,padding:'8px 0',borderBottom:'1px solid var(--border)',background:'#eff6ff',borderRadius:6,paddingLeft:8,marginBottom:4}}>
              ...
              <div style={{flex:1}}>
                ...
                <div style={{fontSize:10,color:'var(--hint)'}}>Auto-generated — always included first</div>
              </div>
              <span style={{fontSize:11,color:'var(--teal)',fontWeight:600,paddingRight:8}}>Included</span>
            </div>

            {workerDocs.length > 0 && <>
              <div style={{fontSize:11,color:'var(--hint)',marginTop:8,marginBottom:6,textTransform:'uppercase',letterSpacing:'0.5px'}}>Documents</div>
              {workerDocs.map(d=>(
            ...
            <div style={{marginTop:16,paddingTop:12,borderTop:'1px solid var(--border)'}}>
              <div style={{fontSize:12,color:'var(--muted)',marginBottom:10}}>{selectedDocIds.length} item{selectedDocIds.length!==1?'s':''} selected + cover page</div>
              <button className="btn btn-primary" style={{width:'100%'}} disabled={selectedDocIds.length===0||building} onClick={handleBuildZip}>{building?'Building ZIP...':'⬇ Download Pack as ZIP'}</button>
