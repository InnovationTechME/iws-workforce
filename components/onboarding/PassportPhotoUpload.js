...
export default function PassportPhotoUpload({ worker, template, existing, onCancel, onSaved }) {
  ...
  const handleSave = async () => {
    setError(null)
    if (!file && !existing?.file_url) { setError('Please select an image'); return }
    setSaving(true)
    try {
      await saveOnboardingDoc(worker, 'passport_photo', {
        files: file,
        notes,
  ...
  return (
    <div style={{ background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: 8, padding: 14, marginTop: 6, display: 'flex', flexDirection: 'column', gap: 10 }}>
      <FileField label="Passport photo" accept=".jpg,.jpeg,.png" file={file} onChange={setFile} required />
