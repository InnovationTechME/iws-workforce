...
export async function uploadGeneratedLetter(refNumber, pdfBlob) {
  if (!refNumber || !pdfBlob) throw new Error('Missing generated letter PDF')
  const bucket = 'letter-archive'
  const safeRef = String(refNumber).replace(/[^A-Za-z0-9_-]/g, '_')
  const year = new Date().getFullYear()
  const path = `generated/${year}/${safeRef}.pdf`

  const { error } = await supabase.storage
    .from(bucket)
    .upload(path, pdfBlob, { upsert: false, contentType: 'application/pdf' })

  if (error) throw new Error(`Generated letter upload failed: ${error.message}`)
  return `${bucket}::${path}`
}
