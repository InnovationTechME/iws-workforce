...
function denormalisedWorkerUpdates(docType, payload) {
  ...
  if (docType === 'passport_copy') {
    if (payload.passport_number !== undefined) upd.passport_number = payload.passport_number || null
    if (payload.expiry_date)                   upd.passport_expiry = payload.expiry_date
  }
  if (docType === 'emirates_id') {
    if (payload.eid_number !== undefined)      upd.emirates_id = payload.eid_number || null
    if (payload.expiry_date)                   upd.emirates_id_expiry = payload.expiry_date
  }
  if (docType === 'uae_visa') {
    if (payload.visa_number !== undefined)     upd.visa_number = payload.visa_number || null
    if (payload.expiry_date)                   upd.visa_expiry = payload.expiry_date
  }
  if (docType === 'health_insurance') {
    if (payload.provider !== undefined)        upd.health_insurance_provider = payload.provider || null
    if (payload.expiry_date)                   upd.health_insurance_expiry = payload.expiry_date
  }
  if (docType === 'workmen_compensation') {
    if (payload.provider !== undefined)        upd.workmen_comp_provider = payload.provider || null
    if (payload.policy_reference !== undefined) upd.workmen_comp_policy_ref = payload.policy_reference || null
