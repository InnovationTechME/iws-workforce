...
export async function getAllCertificationsWithWorkers() {
  const { data, error } = await supabase
    .from('certifications')
    .select('*, workers(id, worker_number, full_name, status)')
    .order('expiry_date', { ascending: true })
  if (error) { console.error('getAllCertificationsWithWorkers error:', error); return [] }
  return (data || []).filter(c => c.workers && c.workers.status !== 'inactive')
}
