...
export function payrollLineToTemplateShape(line) {
  ...
  return {
    ...
    iloe_deduction: Number(line.iloe_deduction ?? 0),
    nwnp_deduction: Number(line.nwnp_deduction ?? 0),
    penalty_deductions: Number(line.penalty_deductions ?? 0),
    other_deductions: Number(line.other_deductions ?? 0),
...
export async function generateHTMLPDF(html, { format = 'a4', widthMm = 210, heightMm = 297 } = {}) {
  const html2canvas = (await import('html2canvas')).default
  const jsPDF = (await import('jspdf')).default

  const wrapper = document.createElement('div')
  wrapper.style.cssText =
    `position:absolute;left:-99999px;top:0;width:${widthMm}mm;background:white;`
  ...
  try {
    ...
    if (imgH <= heightMm) {
      pdf.addImage(imgData, 'JPEG', 0, 0, widthMm, imgH, undefined, 'FAST')
    } else {
      let y = 0
      while (y < imgH) {
        if (y > 0) pdf.addPage()
        pdf.addImage(imgData, 'JPEG', 0, -y, widthMm, imgH, undefined, 'FAST')
        y += heightMm
      }
    }
...
export async function generatePayslipPDF(worker, payrollLine, batch) {
  ...
  const html = payslipHTML(worker, pl, batch, {
    ...
    allowanceLines: [],
    deductionLines: [
      pl.nwnp_deduction > 0 ? { label: 'No work / no pay absence', amount: pl.nwnp_deduction } : null,
      pl.penalty_deductions > 0 ? { label: 'Warning / penalty deductions', amount: pl.penalty_deductions } : null,
      pl.iloe_deduction > 0 ? { label: 'ILOE deduction', amount: pl.iloe_deduction } : null,
      pl.other_deductions > 0 ? { label: 'Other deductions', amount: pl.other_deductions } : null,
    ].filter(Boolean),
  })
...
function renderTimesheetHTML(worker, monthLabel, dailyLines) {
  ...
  const rows = dailyLines.map(l => `
    <tr>
      ...
      <td style="padding:4px 8px;border:1px solid #e2e8f0;font-size:9pt;">${l.is_public_holiday ? 'Public Holiday' : (l.is_rest_day ? 'Rest Day' : (l.is_friday ? 'Friday' : ''))}</td>
    </tr>
  `).join('')
