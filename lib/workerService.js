...
import { nextWorkerNumberFromRows, workerNumberPattern } from './workerNumbering'
...
// Next sequential worker number. Defaults to IWS-YYYY-0001, configurable via
// NEXT_PUBLIC_WORKER_NUMBER_* env vars so existing company formats can continue.
export async function getNextWorkerNumber() {
  const pattern = workerNumberPattern()
  const { data, error } = await supabase
    .from('workers')
    .select('worker_number')
    .like('worker_number', pattern)
    .order('worker_number', { ascending: false })

  if (error || !data || data.length === 0) {
    return nextWorkerNumberFromRows([])
  }

  const next = nextWorkerNumberFromRows(data)
  console.log('getNextWorkerNumber: returning', next)
  return next
}
