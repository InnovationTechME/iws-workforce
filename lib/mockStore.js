...
export const addWorkerWithC3Task = (worker) => {
  addWorker(worker)
  if (worker.category !== 'Subcontractor') {
    addTask({
      id: makeId('task'),
...
export function getDashboardMetrics() {
  ...
  return {
    ...
    byCategory: {
      'Direct Employee': store.workers.filter(w=>w.active&&w.category==='Direct Employee').length,
      'Contracted Hourly Worker': store.workers.filter(w=>w.active&&w.category==='Contracted Hourly Worker').length,
...
export function getWorkforceBreakdown() {
  ...
  return {
    total: active.length,
    direct: active.filter(w => w.category === 'Direct Employee').length,
    hourly: active.filter(w => w.category === 'Contracted Hourly Worker').length,
    subcontractor: active.filter(w => w.category === 'Subcontractor').length,
    office: active.filter(w => w.category === 'Office Staff').length,
  }
}

export function getSubcontractorsByCompany() {
  const subs = store.workers.filter(w => w.active && w.category === 'Subcontractor')
  const grouped = {}
  subs.forEach(worker => {
...
export function getWorkerDisplay(workerId) {
  ...
  if (!worker) return { id:workerId, name:'Unknown', display:workerId }
  return { id: worker.worker_number, name: worker.full_name, display: worker.worker_number + ' — ' + worker.full_name }
}
