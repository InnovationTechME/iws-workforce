// NWNP Business Rules — Innovation Technologies LLC
// UAE Labour Law Federal Decree-Law No. 33 of 2021
// Penalty policy requires MOHRE-registered internal work rules
const FLAT_RATE_ABSENT_PENALTY = 100 // AED per day — flat rate/hourly workers
const MOHRE_MAX_PENALTY_DAYS = 5 // Maximum deductible days per month per UAE law
const CONFLICT_AUTO_RESOLVE_DAYS = 5 // Days before default deduction is applied

// Leave Business Rules
const LEAVE_ENTITLEMENT_DAYS = 30
const LEAVE_NON_RETURN_WARNING_DAY_1 = 3
const LEAVE_NON_RETURN_WARNING_DAY_2 = 4
const LEAVE_NON_RETURN_TERMINATION_DAY = 5
...
export function getVisibleWorkers() {
  const blacklistedPassports = store.blacklist.map(b => b.passport_number?.toLowerCase())
  return store.workers.filter(w => !blacklistedPassports.includes(w.passport_number?.toLowerCase()))
}
export function getActiveVisibleWorkers() {
  return getVisibleWorkers().filter(w => w.active)
}
...
export function updateTimesheetLine(id, updates) {
  const idx = store.timesheetLines.findIndex(l => l.id === id)
  if (idx === -1) return null
  store.timesheetLines[idx] = { ...store.timesheetLines[idx], ...updates }
  return store.timesheetLines[idx]
}
...
// Payroll
// DEPRECATED — use getPayrollBatchByMonth(month) instead
export function getPayrollBatch() { return store.payrollBatches[0] || null }
// DEPRECATED — use getPayrollLinesByBatch(batchId) instead
...
// Offers and Onboarding are now Supabase-backed.
// - Offers:    see lib/offerService.js (getOffers, addOffer, updateOffer,
//              acceptOffer, rejectOffer)
// - Onboarding: see lib/onboardingService.js (getOnboardingRecords,
//              upsertOnboarding, completeOnboarding)
...
// Leave eligibility and workflow
export function getLeaveEligibility(workerId) {
  const worker = store.workers.find(w => w.id === workerId)
  if (!worker) return { eligible: false, reason: 'Worker not found' }
  if (worker.category === 'Contract Worker' || worker.category === 'Subcontract Worker') {
    return { eligible: false, reason: 'Contracted hourly workers and subcontractors are not entitled to annual leave under IWS policy.', category_blocked: true }
  }
  if (!worker.joining_date) return { eligible: false, reason: 'No joining date on record' }
  const joinDate = new Date(worker.joining_date)
  const today = new Date(TODAY)
  const monthsServed = (today.getFullYear() - joinDate.getFullYear()) * 12 + (today.getMonth() - joinDate.getMonth())
  const lastApprovedLeave = store.leaveRecords.filter(l => l.worker_id === workerId && l.leave_type === 'annual' && l.status === 'approved').sort((a, b) => new Date(b.end_date) - new Date(a.end_date))[0]
  let monthsSinceLastLeave = monthsServed
  let lastLeaveEndDate = null
  if (lastApprovedLeave) { lastLeaveEndDate = new Date(lastApprovedLeave.end_date); monthsSinceLastLeave = (today.getFullYear() - lastLeaveEndDate.getFullYear()) * 12 + (today.getMonth() - lastLeaveEndDate.getMonth()) }
  if (monthsServed < 12) return { eligible: false, reason: `Worker has only completed ${monthsServed} months. Minimum 12 months required.`, months_served: monthsServed, months_remaining: 12 - monthsServed }
  if (lastApprovedLeave && monthsSinceLastLeave < 12) return { eligible: false, reason: `Must complete 12 months since last leave return. ${12 - monthsSinceLastLeave} months remaining.`, months_served: monthsSinceLastLeave, months_remaining: 12 - monthsSinceLastLeave, last_leave_end: lastApprovedLeave.end_date }
  const cycleStart = lastApprovedLeave ? lastLeaveEndDate : joinDate
  const currentCycleLeave = store.leaveRecords.filter(l => l.worker_id === workerId && l.leave_type === 'annual' && l.status === 'approved' && new Date(l.start_date) >= cycleStart)
  const daysUsedThisCycle = currentCycleLeave.reduce((sum, l) => sum + (l.days_count || 0), 0)
  const daysRemaining = LEAVE_ENTITLEMENT_DAYS - daysUsedThisCycle
  const under24Months = monthsServed < 24
  return { eligible: true, months_served: monthsServed, days_entitlement: LEAVE_ENTITLEMENT_DAYS, days_used: daysUsedThisCycle, days_remaining: daysRemaining, under_24_months: under24Months, requires_ticket_proof: under24Months, requires_salary_hold: true, management_discretion: under24Months, management_note: under24Months ? 'Worker is between 12-24 months service. Leave at management discretion. Salary hold and self-purchased ticket proof mandatory.' : 'Worker has completed 24+ months. Standard leave process applies. Salary hold mandatory.', joining_date: worker.joining_date, last_leave_end: lastApprovedLeave?.end_date || null, cycle_start: cycleStart.toISOString().split('T')[0] }
}
...
export function calculateSalaryHold(workerId) {
  const worker = store.workers.find(w => w.id === workerId)
  if (!worker) return 0
  return (worker.monthly_salary || 0) + (worker.housing_allowance || 0) + (worker.transport_allowance || 0) + (worker.food_allowance || 0)
}

export function confirmWorkerReturn(leaveId, confirmedBy) {
  const leave = store.leaveRecords.find(l => l.id === leaveId)
  if (!leave) return { success: false }
  store.leaveRecords = store.leaveRecords.map(l => l.id === leaveId ? { ...l, status: 'returned', return_confirmed: true, return_confirmed_by: confirmedBy, return_confirmed_at: new Date().toISOString(), salary_hold_released: true, salary_hold_released_at: new Date().toISOString() } : l)
  return { success: true, hold_released: calculateSalaryHold(leave.worker_id) }
}

export function checkNonReturnStatus() {
  const today = new Date(TODAY)
  const alerts = []
  store.leaveRecords.filter(l => l.status === 'approved' && !l.return_confirmed).forEach(leave => {
    if (!leave.expected_return_date) return
    const expectedReturn = new Date(leave.expected_return_date)
    const daysOverdue = Math.floor((today - expectedReturn) / (1000 * 60 * 60 * 24))
    if (daysOverdue <= 0) return
    const worker = store.workers.find(w => w.id === leave.worker_id)
    alerts.push({ leave_id: leave.id, worker_id: leave.worker_id, worker_name: worker?.full_name || 'Unknown', worker_number: worker?.worker_number || '', days_overdue: daysOverdue, expected_return: leave.expected_return_date, action_required: daysOverdue >= LEAVE_NON_RETURN_TERMINATION_DAY ? 'termination_approval' : daysOverdue >= LEAVE_NON_RETURN_WARNING_DAY_2 ? 'warning_2' : daysOverdue >= LEAVE_NON_RETURN_WARNING_DAY_1 ? 'warning_1' : 'monitor', salary_hold: calculateSalaryHold(leave.worker_id) })
  })
  return alerts
}
...
const refCounters = { offer_letter: 2, warning_1st: 2, warning_2nd: 1, warning_final: 1, experience_letter: 1, memo: 2, termination_notice: 1, termination_no_notice: 1, resignation_acceptance: 1, policy_manual: 1 }
const refPrefixes = { offer_letter:'IT-OL', warning_1st:'IT-WL', warning_2nd:'IT-WL', warning_final:'IT-WL', experience_letter:'IT-EL', memo:'IT-ML', termination_notice:'IT-TN', termination_no_notice:'IT-TX', resignation_acceptance:'IT-RA', policy_manual:'IT-WPM' }
...
export const calculateHourlyPay = (worker, date, hours) => {
  ...
  if (isFlat) {
    // Flat rate workers: no OT premium, holiday = rate × 1.50
    const effectiveRate = holiday ? rate * 1.5 : rate
    return { normal_hours: hours, ot_hours: 0, holiday_hours: holiday ? hours : 0, pay: Math.round(hours * effectiveRate * 100) / 100, rate_applied: effectiveRate }
  }
  // Monthly salaried: base hourly = monthly / 30 / 8
  const monthlyRate = worker.monthly_salary || 0
  const baseHourlyRate = monthlyRate / 30 / 8
  const otThreshold = ramadan ? 6 : 8
  if (holiday) {
    // All hours on holiday at 1.50× — these are additional pay on top of monthly salary
    return { normal_hours: 0, ot_hours: 0, holiday_hours: hours, pay: Math.round(hours * baseHourlyRate * 1.5 * 100) / 100, rate_applied: baseHourlyRate * 1.5 }
  }
  // Normal day: hours up to threshold covered by monthly salary, OT hours = additional pay at 1.25×
  ...
  return { normal_hours: normalHours, ot_hours: otHours, holiday_hours: 0, pay: Math.round(otPay * 100) / 100, rate_applied: baseHourlyRate }
}
...
export function calculateNWNPDeduction(worker, absentDate) {
  const isMonthly = worker.category === 'Permanent Staff' || worker.category === 'Office Staff'
  const isFlat = worker.category === 'Contract Worker' || worker.category === 'Subcontract Worker'
  if (isMonthly) {
    const dailyBasic = (worker.monthly_salary || 0) / 30
    const dailyAllowance = ((worker.housing_allowance || 0) + (worker.transport_allowance || 0) + (worker.food_allowance || 0)) / 30
    return { type: 'monthly_nwnp', daily_basic: Math.round(dailyBasic * 100) / 100, daily_allowance: Math.round(dailyAllowance * 100) / 100, total_per_day: Math.round((dailyBasic + dailyAllowance) * 100) / 100 }
  }
  if (isFlat) {
    return { type: 'flat_penalty', daily_basic: 0, flat_penalty: FLAT_RATE_ABSENT_PENALTY, total_per_day: FLAT_RATE_ABSENT_PENALTY }
  }
  return null
}

export function checkWeekendExtensionRule(absentDate) {
  ...
  if (dayOfWeek === 6) {
    ...
    extraDays.push(sunday.toISOString().split('T')[0])
    reason = 'Absent Saturday — Sunday rest day included in deduction (weekend extension rule)'
  }
  if (dayOfWeek === 1) {
    const sunday = new Date(date); sunday.setDate(sunday.getDate() - 1)
    extraDays.push(sunday.toISOString().split('T')[0])
    reason = 'Absent Monday — Sunday rest day included in deduction (weekend extension rule)'
  }
  return { extended: extraDays.length > 0, extra_days: extraDays, reason, requires_conflict_review: extraDays.length > 0 }
}

export function checkHolidayExtensionRule(absentDate) {
  const date = new Date(absentDate)
  ...
  const beforeIsHoliday = isPublicHoliday(dayBeforeStr)
  const afterIsHoliday = isPublicHoliday(dayAfterStr)
  const extraDays = []
  if (beforeIsHoliday) extraDays.push({ date: dayBeforeStr, name: getHolidayName(dayBeforeStr) })
  if (afterIsHoliday) extraDays.push({ date: dayAfterStr, name: getHolidayName(dayAfterStr) })
  return { extended: extraDays.length > 0, extra_days: extraDays, reason: extraDays.length > 0 ? `Absent adjacent to public holiday (${extraDays.map(d=>d.name).join(', ')}) — extension rule applies` : '', requires_conflict_review: extraDays.length > 0 }
}

export function checkMonthlyPenaltyCap(workerId, month, year, newPenaltyAmount) {
  const worker = getWorker(workerId)
  if (!worker) return { capped: false }
  const dailyRate = (worker.monthly_salary || 0) / 30
  const maxPenalty = dailyRate * MOHRE_MAX_PENALTY_DAYS
  const existingPenalties = penaltyDeductions.filter(p => p.worker_id === workerId && p.status === 'confirmed').reduce((sum, p) => sum + p.amount, 0)
  const totalWithNew = existingPenalties + newPenaltyAmount
  return { capped: totalWithNew > maxPenalty, existing_total: existingPenalties, new_total: totalWithNew, cap_amount: Math.round(maxPenalty * 100) / 100, over_by: Math.max(0, Math.round((totalWithNew - maxPenalty) * 100) / 100), max_additional: Math.max(0, Math.round((maxPenalty - existingPenalties) * 100) / 100), warning_message: totalWithNew > maxPenalty ? `⚠ MOHRE cap reached — maximum deductible this month is AED ${Math.round(maxPenalty*100)/100} (${MOHRE_MAX_PENALTY_DAYS} days basic). Excess: AED ${Math.round((totalWithNew-maxPenalty)*100)/100}` : null }
}
...
export function getAttendanceConflicts() { return attendanceConflicts }
export function getConflictsByWorker(workerId) { return attendanceConflicts.filter(c => c.worker_id === workerId) }
export function getPendingConflicts() { return attendanceConflicts.filter(c => c.status === 'pending') }

export function createAttendanceConflict(attendanceRecord, conflictType, defaultDeduction) {
  const worker = getWorker(attendanceRecord.worker_id)
  const conflict = {
    id: makeId('conf'), worker_id: attendanceRecord.worker_id, worker_name: worker?.full_name || 'Unknown', worker_number: worker?.worker_number || '',
    attendance_id: attendanceRecord.id, absent_date: attendanceRecord.date, conflict_type: conflictType,
    description: conflictType === 'weekend_extension' ? 'Weekend extension rule applied — confirm if Sunday was rest day for this team'
      : conflictType === 'holiday_extension' ? 'Public holiday adjacent absence — confirm if extension applies'
      : 'Flat rate penalty applied — review if authorised absence',
    default_deduction: defaultDeduction, applied_deduction: defaultDeduction,
    status: 'pending', hr_clarification: null, hr_clarified_by: null, hr_clarified_at: null,
    ops_approved_by: null, ops_approved_at: null, resolution: null, final_deduction: null,
    created_at: new Date().toISOString(),
    auto_resolve_date: new Date(Date.now() + CONFLICT_AUTO_RESOLVE_DAYS * 24 * 60 * 60 * 1000).toISOString()
  }
  attendanceConflicts = [...attendanceConflicts, conflict]
  return conflict
}

export function submitHRClarification(conflictId, clarification, hrName) {
  attendanceConflicts = attendanceConflicts.map(c => c.id !== conflictId ? c : { ...c, status: 'hr_clarified', hr_clarification: clarification, hr_clarified_by: hrName, hr_clarified_at: new Date().toISOString() })
}

export function approveConflictResolution(conflictId, opsName, resolution, finalDeduction) {
  attendanceConflicts = attendanceConflicts.map(c => {
    if (c.id !== conflictId) return c
    if (resolution === 'waived') { penaltyDeductions = penaltyDeductions.filter(p => p.conflict_id !== conflictId) }
    else if (resolution === 'reduced') { penaltyDeductions = penaltyDeductions.map(p => p.conflict_id === conflictId ? { ...p, amount: finalDeduction } : p) }
    return { ...c, status: 'resolved', ops_approved_by: opsName, ops_approved_at: new Date().toISOString(), resolution, final_deduction: finalDeduction, applied_deduction: finalDeduction }
  })
}

export function autoResolveExpiredConflicts() {
  const now = new Date()
  const notes = []
  attendanceConflicts = attendanceConflicts.map(c => {
    if (c.status !== 'pending' && c.status !== 'hr_clarified') return c
    if (new Date(c.auto_resolve_date) > now) return c
    notes.push({ worker_id: c.worker_id, worker_name: c.worker_name, conflict_id: c.id, note: `Unresolved attendance conflict from ${c.absent_date} — weekend/holiday extension waived (auto-resolved). Review in this payroll period.`, month: new Date(c.absent_date).toLocaleString('en-GB', { month: 'long' }), year: new Date(c.absent_date).getFullYear() })
    return { ...c, status: 'auto_resolved', resolution: 'reduced', final_deduction: 0, applied_deduction: 0, auto_resolved_at: now.toISOString(), auto_resolve_note: 'Auto-resolved after 5 days — lower amount applied. Carry-over note created.' }
  })
  notes.forEach(n => addCarryOverNote(n))
  return notes
}
...
let carryOverNotes = []
export function getCarryOverNotes() { return carryOverNotes }
export function getCarryOverNotesByWorker(workerId) { return carryOverNotes.filter(n => n.worker_id === workerId) }
export function getPendingCarryOverNotes() { return carryOverNotes.filter(n => !n.resolved) }
export function addCarryOverNote(note) { carryOverNotes = [...carryOverNotes, { ...note, id: makeId('con'), resolved: false, created_at: new Date().toISOString(), resolved_at: null, resolved_by: null }] }
export function resolveCarryOverNote(id, resolvedBy) { carryOverNotes = carryOverNotes.map(n => n.id === id ? { ...n, resolved: true, resolved_at: new Date().toISOString(), resolved_by: resolvedBy } : n) }
...
export function getDashboardMetrics() {
  const activeWorkers = getVisibleWorkers().filter(w => w.active)
  ...
  // Onboarding is Supabase-backed; synchronous dashboard stat is no longer
  // computed from a local store. Consumers that need this figure should call
  // getOnboardingRecords() from lib/onboardingService.js (async).
  const onboardingActive = 0
  return {
    ...
    byCategory: {
      'Permanent Staff': getVisibleWorkers().filter(w=>w.active&&w.category==='Permanent Staff').length,
      'Contract Worker': getVisibleWorkers().filter(w=>w.active&&w.category==='Contract Worker').length,
      'Subcontractor': getVisibleWorkers().filter(w=>w.active&&w.category==='Subcontractor').length,
      'Office Staff': getVisibleWorkers().filter(w=>w.active&&w.category==='Office Staff').length,
    }
  }
}
...
export function getInboxItems() {
  ...
  const workerMap = Object.fromEntries(visible.map(w => [w.id, w]))
  const enrich = (arr) => arr.filter(item => visibleIds.has(item.worker_id)).map(item => ({ ...item, worker_name: workerMap[item.worker_id]?.full_name || 'Unknown', worker_number: workerMap[item.worker_id]?.worker_number || '-' }))
  return {
    ...
    // Onboarding blockers now live in Supabase; the HR Inbox page should
    // query getOnboardingRecords() from lib/onboardingService.js directly.
    onboardingBlockers: [],
...
export function getPackCoverage() {
  const requiredTypes = ['passport', 'photo', 'uae_visa', 'emirates_id', 'workers_compensation']
  return getVisibleWorkers().map(worker => {
...
export function getPayrollHistoryByYear(year) {
  return store.payrollBatches
    .filter(b => b.year === year)
    .sort((a, b) => b.month_number - a.month_number)
}

export function getPayrollYearSummary(year) {
  const batches = getPayrollHistoryByYear(year)
  return {
    year,
    total_gross: batches.reduce((s, b) => s + (b.total_gross || 0), 0),
    total_net: batches.reduce((s, b) => s + (b.total_net || 0), 0),
    months_on_record: batches.length,
    all_locked: batches.every(b => b.locked),
    batches
  }
}

export function getRetentionStatus(batch) {
  if (!batch.locked_at) return { status:'unlocked', safe:false, message:'Not yet locked — record not protected' }
  const lockedDate = new Date(batch.locked_at)
  const now = new Date()
  const yearsHeld = (now - lockedDate) / (1000*60*60*24*365)
  const retentionExpiry = new Date(lockedDate)
  retentionExpiry.setFullYear(retentionExpiry.getFullYear() + 5)
  const yearsRemaining = Math.max(0, (retentionExpiry - now) / (1000*60*60*24*365))
  const expiryLabel = retentionExpiry.toLocaleDateString('en-GB', { month:'long', year:'numeric' })
  return { status:'locked', safe:true, locked_at:batch.locked_at, years_held:Math.round(yearsHeld*10)/10, years_remaining:Math.round(yearsRemaining*10)/10, expiry_date:retentionExpiry.toISOString().split('T')[0], message:'Retained until '+expiryLabel }
}
