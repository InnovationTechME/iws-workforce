...
function mapWarning(row) {
  ...
  const statusToUi = { active: 'open', appealed: 'monitoring', expired: 'closed' }
  return {
    ...row,
...
function dbPayload(data, { forInsert = false } = {}) {
  ...
  const statusToDb = { open: 'active', monitoring: 'appealed', closed: 'expired' }
  if (payload.status && statusToDb[payload.status]) payload.status = statusToDb[payload.status]
