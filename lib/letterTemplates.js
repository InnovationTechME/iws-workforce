...
<table style="width:100%;border-collapse:collapse;margin-bottom:14px;font-size:12px">
  ...
  <tr><td style="padding:5px 0;color:#475569">Rest day</td><td style="padding:5px 0">Sunday (or as assigned per worker)</td></tr>
...
export const payslipHTML = (worker, payrollLine, batch, { leaveBalance, ytd, ramadanActive, allowanceLines, deductionLines }) => {
  ...
  let earningsRows = ''
  if (isFlat) {
    const totalFlatHours = (pl.normal_hours||0)+(pl.ot_hours||0)+(pl.holiday_hours||0)
    const stdPay = Math.round(totalFlatHours * (worker.hourly_rate||0) * 100)/100
    earningsRows = `
      ${row('Hourly rate', fmtAED(worker.hourly_rate) + '/hr')}
      ${row('Hours worked', totalFlatHours.toFixed(1) + ' h')}
...
export const exitClearanceHTML = (worker, offboarding, refNumber, date) => {
  const fullName = worker.full_name || '---'
  const content = `
<div class="letter-title">Exit Clearance Form</div>
<p class="body-text">This form records final clearance for <strong>${fullName}</strong> before the worker file is closed in IWS.</p>
<div class="info-box"><table>
  <tr><td>Worker name</td><td>${fullName}</td></tr>
  <tr><td>Worker number</td><td>${worker.worker_number || '---'}</td></tr>
  <tr><td>Trade / role</td><td>${worker.trade_role || '---'}</td></tr>
  <tr><td>Reason for exit</td><td>${offboarding.reason || '---'}</td></tr>
  <tr><td>Joining date</td><td>${worker.joining_date || '---'}</td></tr>
  <tr><td>Last working date</td><td>${offboarding.last_working_date || '---'}</td></tr>
</table></div>
<div class="terms">
<h4>Clearance Checklist</h4>
<ul>
  <li>Company property, PPE, tools, access cards, and site passes returned or charged.</li>
  <li>Timesheets and attendance records reviewed up to the last working date.</li>
  <li>Final payslip prepared by Accounts / Payroll.</li>
  <li>End-of-service / final settlement reviewed and approved where applicable.</li>
  <li>Visa, labour card, medical insurance, and C3/WPS actions completed or initiated.</li>
</ul>
</div>
<div class="sig-block">
  <div class="sig-col"><div class="sig-label">Operations / Site Clearance</div><div class="sig-line">Name, Signature &amp; Date</div></div>
  <div class="sig-col" style="text-align:right"><div class="sig-label">HR Clearance</div><div class="sig-line">Name, Signature &amp; Date</div></div>
</div>
<div class="sig-block" style="margin-top:22px">
  <div class="sig-col"><div class="sig-label">Accounts / Payroll Clearance</div><div class="sig-line">Name, Signature &amp; Date</div></div>
  <div class="sig-col" style="text-align:right"><div class="sig-label">Worker Acknowledgement</div><div class="sig-name">${fullName}</div><div class="sig-line">Signature &amp; Date</div></div>
</div>`
  return shell(refNumber, date, content)
}

export const finalSettlementHTML = (worker, offboarding, payrollLines, settlement, refNumber, date) => {
  const fullName = worker.full_name || '---'
  const latestLine = payrollLines?.[0]
  const latestBatch = latestLine?.batch
  const amount = Number(settlement?.amount || offboarding.eos_amount || 0)
  const gratuity = Number(settlement?.gratuity || 0)
  const finalPayroll = Number(settlement?.finalPayroll || 0)
  const leavePay = Number(settlement?.leavePay || 0)
  const deductions = Number(settlement?.deductions || 0)
  const manualAdjustments = Number(settlement?.manualAdjustments || 0)
  const fmt = n => `AED ${Number(n || 0).toLocaleString('en-AE', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`
  const content = `
<div class="letter-title">Full &amp; Final Settlement Statement</div>
<p class="body-text">This statement records the final settlement prepared for <strong>${fullName}</strong>. Amounts must be reviewed against approved payroll, attendance, leave, deductions, and management approval before payment.</p>
<div class="info-box"><table>
  <tr><td>Worker name</td><td>${fullName}</td></tr>
  <tr><td>Worker number</td><td>${worker.worker_number || '---'}</td></tr>
  <tr><td>Reason for exit</td><td>${offboarding.reason || '---'}</td></tr>
  <tr><td>Joining date</td><td>${worker.joining_date || '---'}</td></tr>
  <tr><td>Last working date</td><td>${offboarding.last_working_date || '---'}</td></tr>
  <tr><td>Payroll type</td><td>${latestLine?.payroll_type || worker.payroll_type || worker.category || '---'}</td></tr>
  <tr><td>Latest payroll on file</td><td>${latestBatch ? `${latestBatch.month_label || `${latestBatch.month}/${latestBatch.year}`} (${latestBatch.status || 'status n/a'})` : 'No payroll line found'}</td></tr>
</table></div>
<table style="width:100%;border-collapse:collapse;margin:16px 0;font-size:12px">
  <tr style="background:#f8fafc"><th style="text-align:left;padding:8px;border:1px solid #e2e8f0">Component</th><th style="text-align:right;padding:8px;border:1px solid #e2e8f0">Amount</th></tr>
  <tr><td style="padding:8px;border:1px solid #e2e8f0">EOS gratuity / service benefit</td><td style="text-align:right;padding:8px;border:1px solid #e2e8f0">${fmt(gratuity)}</td></tr>
  <tr><td style="padding:8px;border:1px solid #e2e8f0">Final payroll / salary due</td><td style="text-align:right;padding:8px;border:1px solid #e2e8f0">${fmt(finalPayroll)}</td></tr>
  <tr><td style="padding:8px;border:1px solid #e2e8f0">Leave encashment / other earnings</td><td style="text-align:right;padding:8px;border:1px solid #e2e8f0">${fmt(leavePay)}</td></tr>
  <tr><td style="padding:8px;border:1px solid #e2e8f0">Manual adjustments</td><td style="text-align:right;padding:8px;border:1px solid #e2e8f0">${fmt(manualAdjustments)}</td></tr>
  <tr><td style="padding:8px;border:1px solid #e2e8f0">Deductions / recoveries</td><td style="text-align:right;padding:8px;border:1px solid #e2e8f0">-${fmt(deductions)}</td></tr>
  <tr style="background:#f0fdfa"><td style="padding:9px;border:1px solid #99f6e4;font-weight:700">Net full &amp; final settlement</td><td style="text-align:right;padding:9px;border:1px solid #99f6e4;font-weight:700;color:#0f766e">${fmt(amount)}</td></tr>
</table>
<div style="background:#fffbeb;border:1px solid #fcd34d;border-radius:6px;padding:10px 14px;margin:14px 0;font-size:12px;color:#92400e">Payroll note: this document is generated from IWS offboarding and payroll records. Any manual amount must be reconciled with the approved final payslip before payment.</div>
<div class="sig-block">
  <div class="sig-col"><div class="sig-label">Prepared by HR / Payroll</div><div class="sig-line">Name, Signature &amp; Date</div></div>
  <div class="sig-col" style="text-align:right"><div class="sig-label">Management Approval</div><div class="sig-line">Name, Signature &amp; Date</div></div>
</div>
<div class="sig-block" style="margin-top:22px">
  <div class="sig-col"><div class="sig-label">Payment Confirmation</div><div class="sig-line">Accounts Signature &amp; Date</div></div>
  <div class="sig-col" style="text-align:right"><div class="sig-label">Worker Receipt &amp; Acknowledgement</div><div class="sig-name">${fullName}</div><div class="sig-line">Signature &amp; Date</div></div>
</div>`
  return shell(refNumber, date, content)
}
...
<p style="font-size:12px;line-height:1.85;color:#334155;margin-bottom:6px;">
  ...
  Rest day: Sunday or as assigned by your supervisor.<br/>
...
<p style="font-size:12px;line-height:1.85;color:#334155;margin-bottom:6px;">
  ...
  Rest day / public holiday = 1.50×. Hourly rate = monthly salary ÷ 30 ÷ 8.
  <strong>Flat-rate workers:</strong> all hours at flat rate — no OT premium.
  Public holidays remain flat rate only for flat-rate workers.<br/>
