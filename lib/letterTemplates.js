...
export const payslipHTML = (worker, payrollLine, batch, { leaveBalance, ytd, ramadanActive, allowanceLines, deductionLines }) => {
  const refNumber = `IT-PS-${batch.month_label?.replace(/\s+/g,'-') || batch.month}-${worker.worker_number}`
  ...
  const [pYear, pMonth] = (batch.month||'2026-04').split('-').map(Number)
  const daysInPeriod = new Date(pYear, pMonth, 0).getDate()
  ...
  const paymentLabel = (!worker.payment_method || worker.payment_method === 'WPS') ? 'C3 Card (WPS)' : worker.payment_method === 'Non-WPS' ? 'C3 Card (Non-WPS)' : 'Cash'

  const fmtAED = (v) => 'AED ' + (v||0).toLocaleString(undefined,{minimumFractionDigits:2,maximumFractionDigits:2})

  // ─── A5 payslip shell ───
  const LOGO_SMALL = LOGO_BASE64 ? `<img src="${LOGO_BASE64}" style="width:32px;height:32px;object-fit:contain" alt="iN"/>` : `<div style="width:32px;height:32px;border-radius:50%;background:#1e3a8a;display:flex;align-items:center;justify-content:center;color:#fff;font-weight:800;font-size:13px">iN</div>`

  let body = ''
  if (isFlat) {
    // ═══ TYPE 2 — FLAT RATE / SUBCONTRACT PAYSLIP ═══
    ...
    const totalEarnings = stdPay + Math.round((pl.holiday_hours||0)*(worker.hourly_rate||0)*100)/100 + holPremium
    body = `
<!-- Worker Info -->
<div style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:3px;padding:6px 8px;margin-bottom:7px">
  <table style="width:100%;border-collapse:collapse;font-size:7.5pt"><tr><td style="padding:2px 0;color:#475569;width:35%">Name</td><td style="font-weight:600">${worker.full_name}</td><td style="color:#475569;width:20%">Worker ID</td><td style="font-family:monospace">${worker.worker_number}</td></tr>
  <tr><td style="padding:2px 0;color:#475569">Trade</td><td>${worker.trade_role||'—'}</td><td style="color:#475569">Nationality</td><td>${worker.nationality||'—'}</td></tr>
  <tr><td style="padding:2px 0;color:#475569">Month</td><td>${monthName}</td><td style="color:#475569">Payment</td><td>${paymentLabel}</td></tr></table>
</div>
<!-- Earnings -->
<div style="margin-bottom:6px"><div style="font-size:8pt;font-weight:700;color:#1e3a8a;text-transform:uppercase;letter-spacing:1px;border-bottom:1.5px solid #1e3a8a;padding-bottom:2px;margin-bottom:3px">Earnings</div>
<table style="width:100%;border-collapse:collapse;font-size:8.5pt">
<tr><td style="padding:3px 0;color:#475569">Hourly Rate</td><td style="text-align:right">${fmtAED(worker.hourly_rate)}/hr</td></tr>
<tr><td style="padding:3px 0;color:#475569">Hours Worked</td><td style="text-align:right">${(pl.normal_hours||0)+(pl.holiday_hours||0)}h</td></tr>
<tr><td style="padding:3px 0;color:#475569">Standard Pay (${pl.normal_hours||0}h × ${fmtAED(worker.hourly_rate)})</td><td style="text-align:right;font-weight:500">${fmtAED(stdPay)}</td></tr>
${(pl.holiday_hours||0)>0 ? `<tr><td style="padding:3px 0;color:#dc2626">Holiday Premium (${pl.holiday_hours}h × 0.50 add.)</td><td style="text-align:right;color:#dc2626">${fmtAED(holPremium)}</td></tr>` : ''}
<tr><td colspan="2" style="border-top:1px solid #e2e8f0;padding-top:4px"></td></tr>
<tr><td style="padding:3px 0;font-weight:600">Total Earnings</td><td style="text-align:right;font-weight:700">${fmtAED(grossTotal)}</td></tr>
</table></div>
${totalDeductions > 0 ? `
<div style="margin-bottom:6px"><div style="font-size:8pt;font-weight:700;color:#dc2626;text-transform:uppercase;letter-spacing:1px;border-bottom:1.5px solid #dc2626;padding-bottom:2px;margin-bottom:3px">Deductions</div>
<table style="width:100%;border-collapse:collapse;font-size:8.5pt">
${(deductionLines||[]).map(d => `<tr><td style="padding:2px 0;color:#dc2626">${d.label}</td><td style="text-align:right;color:#dc2626">- ${fmtAED(d.amount)}</td></tr>`).join('')}
<tr><td colspan="2" style="border-top:1px solid #e2e8f0;padding-top:3px"></td></tr>
<tr><td style="padding:2px 0;font-weight:600;color:#991b1b">Total Deductions</td><td style="text-align:right;font-weight:700;color:#991b1b">- ${fmtAED(totalDeductions)}</td></tr>
</table></div>` : ''}
<!-- NET PAY -->
<div style="background:#0f172a;border-radius:4px;padding:8px 10px;margin-bottom:4px;display:flex;justify-content:space-between;align-items:center">
  <span style="font-size:11pt;font-weight:700;color:#fff;text-transform:uppercase">Net Pay</span>
  <span style="font-size:16pt;font-weight:800;color:#60a5fa">${fmtAED(pl.net_pay)}</span>
</div>
<div style="font-size:7pt;color:#94a3b8;margin-bottom:6px">${(!worker.payment_method||worker.payment_method==='WPS') ? 'Flat rate contract · WPS via C3 Card' : worker.subcontractor_company ? 'Flat rate contract · Non-WPS · Own visa' : 'Flat rate contract · '+paymentLabel}</div>
<!-- Digital delivery notice -->
<div style="border-top:1px solid #e2e8f0;margin-top:8px;padding-top:6px;text-align:center">
  <div style="font-size:7.5pt;color:#64748b;margin-bottom:3px">This payslip was digitally generated by IWS — Innovation Workforce System</div>
  <div style="font-size:7.5pt;color:#64748b;margin-bottom:3px">Delivered to: ${worker.email || 'worker registered email'}</div>
  <div style="font-size:6.5pt;color:#94a3b8">System Ref: ${refNumber} · Generated: ${date}</div>
</div>
<div style="display:flex;justify-content:flex-end;margin-top:6px">
  <div style="text-align:right">
    <div style="font-size:7.5pt;font-weight:700;color:#0f172a;margin-bottom:2px">For Innovation Technologies LLC</div>
    <div style="font-size:7pt;color:#475569">Authorised Signatory: ____________________</div>
  </div>
</div>`
  } else {
    // ═══ TYPE 1 — MONTHLY SALARIED PAYSLIP ═══
...
<!-- Worker Info -->
<div style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:3px;padding:6px 8px;margin-bottom:7px">
  <table style="width:100%;border-collapse:collapse;font-size:7.5pt"><tr><td style="padding:2px 0;color:#475569;width:35%">Employee Name</td><td style="font-weight:600">${worker.full_name}</td><td style="color:#475569;width:20%">Worker ID</td><td style="font-family:monospace">${worker.worker_number}</td></tr>
  <tr><td style="padding:2px 0;color:#475569">Trade / Position</td><td>${worker.trade_role||'—'}</td><td style="color:#475569">Nationality</td><td>${worker.nationality||'—'}</td></tr>
  <tr><td style="padding:2px 0;color:#475569">Pay Period</td><td>${monthName}</td><td style="color:#475569">Payment</td><td>${paymentLabel}</td></tr></table>
</div>
<!-- Earnings -->
<div style="margin-bottom:6px"><div style="font-size:8pt;font-weight:700;color:#1e3a8a;text-transform:uppercase;letter-spacing:1px;border-bottom:1.5px solid #1e3a8a;padding-bottom:2px;margin-bottom:3px">Earnings</div>
<table style="width:100%;border-collapse:collapse;font-size:8.5pt">
<tr><td style="padding:3px 0;color:#475569">Basic Salary</td><td style="text-align:right;font-weight:600">${fmtAED(worker.monthly_salary)}</td></tr>
${isOffice ? `
${(worker.housing_allowance||0)>0 ? `<tr><td style="padding:2px 0;color:#475569;padding-left:12px;font-size:9px">Housing Allowance</td><td style="text-align:right;font-size:9px">${fmtAED(worker.housing_allowance)}</td></tr>` : ''}
${(worker.transport_allowance||0)>0 ? `<tr><td style="padding:2px 0;color:#475569;padding-left:12px;font-size:9px">Transport Allowance</td><td style="text-align:right;font-size:9px">${fmtAED(worker.transport_allowance)}</td></tr>` : ''}
${(worker.food_allowance||0)>0 ? `<tr><td style="padding:2px 0;color:#475569;padding-left:12px;font-size:9px">Food Allowance</td><td style="text-align:right;font-size:9px">${fmtAED(worker.food_allowance)}</td></tr>` : ''}
` : totalAllowance > 0 ? `<tr><td style="padding:3px 0;color:#475569">Allowance</td><td style="text-align:right">${fmtAED(totalAllowance)}</td></tr>` : ''}
${(pl.ot_hours||0) > 0 ? `<tr><td style="padding:3px 0;color:#d97706">OT Weekday (${pl.ot_hours}h × ${fmtAED(baseRate)} × 1.25)</td><td style="text-align:right;color:#d97706">${fmtAED(ot1Pay)}</td></tr>` : ''}
${(pl.holiday_hours||0) > 0 ? `<tr><td style="padding:3px 0;color:#dc2626">OT Fri/Holiday (${pl.holiday_hours}h × ${fmtAED(baseRate)} × 1.50)</td><td style="text-align:right;color:#dc2626">${fmtAED(ot2Pay)}</td></tr>` : ''}
${ramadanActive ? '<tr><td colspan="2" style="padding:2px 0;font-size:8px;color:#7c3aed;font-style:italic">* Ramadan mode applied — 6hr threshold</td></tr>' : ''}
<tr><td colspan="2" style="border-top:1px solid #e2e8f0;padding-top:4px"></td></tr>
<tr><td style="padding:3px 0;font-weight:600">Gross Earnings</td><td style="text-align:right;font-weight:700">${fmtAED(grossTotal)}</td></tr>
</table></div>
${totalDeductions > 0 ? `
<div style="margin-bottom:6px"><div style="font-size:8pt;font-weight:700;color:#dc2626;text-transform:uppercase;letter-spacing:1px;border-bottom:1.5px solid #dc2626;padding-bottom:2px;margin-bottom:3px">Deductions</div>
<table style="width:100%;border-collapse:collapse;font-size:8.5pt">
...
<!-- NET PAY -->
<div style="background:#0f172a;border-radius:4px;padding:8px 10px;margin-bottom:4px;display:flex;justify-content:space-between;align-items:center">
  <span style="font-size:11pt;font-weight:700;color:#fff;text-transform:uppercase">Net Pay / صافي الراتب</span>
  <span style="font-size:16pt;font-weight:800;color:#60a5fa">${fmtAED(pl.net_pay)}</span>
</div>
<div style="font-size:7pt;color:#64748b;font-style:italic;margin-bottom:6px">${(!worker.payment_method||worker.payment_method==='WPS') ? 'Payment via C3 Card (Endered/WPS)' : worker.payment_method==='Cash' ? 'Payment: Cash' : 'Payment: '+paymentLabel}</div>
<!-- Digital delivery notice -->
<div style="border-top:1px solid #e2e8f0;margin-top:8px;padding-top:6px;text-align:center">
  <div style="font-size:7.5pt;color:#64748b;margin-bottom:3px">This payslip was digitally generated by IWS — Innovation Workforce System</div>
  <div style="font-size:7.5pt;color:#64748b;margin-bottom:3px">Delivered to: ${worker.email || 'worker registered email'}</div>
...
*{margin:0;padding:0;box-sizing:border-box}
body{font-family:'Barlow',sans-serif;font-size:8.5pt;color:#1a1a2e;background:#fff}
@media print { @page{size:A5 portrait;margin:6mm} body.printing-payslip *{visibility:hidden} body.printing-payslip #payslip-print-area, body.printing-payslip #payslip-print-area *{visibility:visible} body.printing-payslip #payslip-print-area{position:fixed;top:0;left:0;width:148mm;font-size:8.5pt;background:white} body.printing-payslip .no-print{display:none!important} }
</style></head><body>
<div id="payslip-print-area" class="payslip-print-area" style="width:148mm;min-height:200mm;max-height:210mm;margin:0 auto;padding:8mm;position:relative;box-sizing:border-box;overflow:hidden">
<!-- Header -->
<div style="display:flex;align-items:center;margin-bottom:4px">
  ${LOGO_SMALL}
  <div style="flex:1;margin-left:8px"><span style="font-size:11px;font-weight:700;color:#0f172a">${CO.name}</span></div>
  <div style="text-align:right"><div style="font-size:7pt;color:#64748b">${CO.phone} · ${CO.email}</div><div style="font-size:6.5pt;color:#94a3b8;margin-top:1px">Ref: ${psRef}</div></div>
</div>
<div style="border-top:2px solid #1e3a8a;margin-bottom:6px"></div>
<!-- PAYSLIP heading -->
<div style="text-align:center;padding:10px 0 8px 0;border-bottom:1px solid #e2e8f0;margin-bottom:12px">
  <span style="font-size:15pt;font-weight:700;letter-spacing:3px;color:#0f172a;text-transform:uppercase">PAYSLIP</span>
  <span style="font-size:10pt;color:#94a3b8;margin-left:8px;font-weight:400">/ كشف الراتب</span>
  <div style="font-size:8.5pt;color:#64748b;margin-top:3px;font-weight:400">${monthName}</div>
</div>
${body}
<!-- Footer -->
<div style="position:absolute;bottom:6mm;left:8mm;right:8mm;font-size:6.5pt;color:#94a3b8;text-align:center;line-height:1.5;border-top:1px solid #e2e8f0;padding-top:3px">
${CO.name} · Workhub M21 Musaffah Abu Dhabi · Licence: ${CO.licence} · VAT: ${CO.vat} · MOHRE: ${CO.mohre} · Confidential
</div>
</div></body></html>`
}
