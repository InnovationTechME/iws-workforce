  ...
  worksheet.eachRow({ includeEmpty: true }, row => {
    ...
    for (let i = 1; i <= max; i++) {
      ...
      if (value && typeof value === 'object') {
        values.push(value.result ?? value.text ?? value.richText?.map(part => part.text).join('') ?? value.formula ?? '')
      } else {
        values.push(value ?? '')
      }
    }
...
export async function parseClientTimesheet(file) {
  ...
  const sheetName = worksheet?.name || 'Innovation Direct'
  const jsonData = worksheet ? worksheetToRows(worksheet) : []
